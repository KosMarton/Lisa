...
namespace YAML {
	...
	template<>
	struct convert<Lisa::UUID>
	{
		static Node encode(const Lisa::UUID& uuid)
		{
			Node node;
		...
		static bool decode(const Node& node, Lisa::UUID& uuid)
		{
...
#define READ_SCRIPT_FIELD(FieldType, Type)           \
	...
	{
		LS_CORE_ASSERT(entity.HasComponent<IDComponent>());
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		LS_CORE_TRACE("Deserializing scene '{0}'", sceneName);
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				LS_CORE_TRACE("Deserialized entity with ID = {0}, name = {1}", uuid, name);
